...
class PFMendeleyAutocompleteAPI extends ApiBase {
	...
	public function execute() {
		...
		// Transport failure (null) or an OAuth error payload - nothing usable
		// to iterate over; report an empty result instead of crashing below.
		if ( !is_array( $result ) || isset( $result['errorId'] ) ) {
			$this->getResult()->addValue( 'result', "autocomplete_results", [] );
			return;
		}
		...
		foreach( $result as $row ) {
			...
			$authors = array();
			foreach( (array)( $row['authors'] ?? [] ) as $author ) {
				if ( !is_array( $author ) ) {
					continue;
				}
